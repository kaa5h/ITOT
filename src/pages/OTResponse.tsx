...
import { ChevronDown, ChevronUp, Send, AlertTriangle, CheckCircle, Plus, Trash2 } from 'lucide-react';
...
const OTResponse: React.FC = () => {
  ...
  const template = templates.find((t) => t.protocol === request?.connection?.protocol);
  ...
  useEffect(() => {
    if (request) {
      ...
      setConnectionHost(request.connection?.host || '');
      setConnectionPort(request.connection?.port?.toString() || '502');
    }
  }, [request]);
  ...
  const getTotalCompletion = () => {
    const connectionFilled = connectionHost && connectionPort ? 1 : 0;
    const connectionTotal = request.connection?.host ? 0 : 1;
    ...
    return {
      filled: connectionFilled + endpointFilled,
      total: connectionTotal + endpointTotal,
    };
  };
  ...
  const handleSendMessage = () => {
    if (messageText.trim()) {
      ...
      // Update status to discussion-active if not already
  ...
  const handleSubmitIssue = () => {
    if (issueType && issueDescription.trim()) {
      ...
      // Add issue message to conversation
      const issueMessage: Message = {
        id: 'msg-' + Date.now(),
        timestamp: new Date().toISOString(),
        from: currentUser.name,
        role: currentUser.role,
        message: `⚠️ Issue with endpoint: ${issueDescription}`,
        issueFlagged: true,
      };
      addMessage(request.id, issueMessage);

      // Update request status and endpoints
  ...
  const handleSaveDraft = () => {
    const connection = {
      ...request.connection,
      host: connectionHost,
      port: parseInt(connectionPort) || 502,
      otFilled: true,
    };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border border-gray-200 rounded-lg h-[calc(100vh-20rem)] flex flex-col">
            {/* Chat Header */}
